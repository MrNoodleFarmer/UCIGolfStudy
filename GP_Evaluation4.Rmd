...
splot5<-ggplot(boysVsTemp,aes(x = x,y = y)) +geom_point(colour='blue') +geom_point(data=girlsVsTemp,colour='red') + xlab("Avg Temperature (F)") + ylab("Golfer Population") + ggtitle("Junior Population Vs Avg Annual Temperature (Boys = Blue, Girls = Red)")
...
## Evaluation of Total Player Population Vs Average Annual Temperature


### Map of Average Temperature in Fahrenheit

```{r weatherTemp, echo=FALSE}
print(p14)
print(p17)
```


#### There isn't really a noticable relationship just by observing maps. We could argue that temperature doesn't make a significant affect in where people chose to play golf.
...
```{r lm7, echo=FALSE}
plot(y ~ x, data = boysVsTemp, ylab = "Golfer Population", xlab = "Temperature (F)")
  abline(boysVsTempLm)
```
...
```{r lm8, echo=FALSE}
plot(y ~ x, data = girlsVsTemp, ylab = "Golfer Population", xlab = "Temperature (F)")
  abline(girlsVsTempLm)
```

#### Analyzing Residuals and Testing Regression:

```{r lm8_1, echo=FALSE}
print(summary(girlsVsTempLm))
plot(girlsVsTempLm)
```
...
#### Weather is another factor that logically seems like a big factor to the popularity of golf. Not only is it an outside sport, but golf courses rely on lots of water, and players mostly want to experience a nice weather as they play 18 holes, so it's assumed that there is a relationship between temperature and golf popularity. The p value of the our linear model for both boys and girls was below 0.05, which means that changes in temperature correlate to changes in the amount of golf players. However, the scatterplot with the regression line does not look fitting, as we can visually see some high residuals. 

#### Based on how far we analyzed the reisuals and linear regression line, we can say that the temperature in each state is a significant factor that affects golf popularity within that area in a linear scale. However, the problem is that a linear regression model shouldn't be appropriate for anything weather-related, because it should not really be a linear relationship - there is a middle, preferred temperature, while extremes are unfavored. Therefore, we are very restricted by only doing a linear regression model, and we can just barely discern temperature as a significant factor in golf popularity.
