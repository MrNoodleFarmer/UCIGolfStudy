...
splot3<-ggplot(boysVsStateInc,aes(x = x,y = y)) +geom_point(colour='blue') +geom_point(data=girlsVsStateInc,colour='red') + xlab("State Income") + ylab("Golfer Population")
...
splot4<-ggplot(boysVsCourses,aes(x = x,y = y)) +geom_point(colour='blue') +geom_point(data=girlsVsCourses,colour='red') + xlab("Golf Courses") + ylab("Golfer Population")
...
splot5<-ggplot(boysVsTemp,aes(x = x,y = y)) +geom_point(colour='blue') +geom_point(data=girlsVsTemp,colour='red') + xlab("Avg Temperature Fahrenheit") + ylab("Golfer Population")
...
splot6<-ggplot(boysVsPrecip,aes(x = x,y = y)) +geom_point(colour='blue') +geom_point(data=girlsVsPrecip,colour='red') + xlab("Precipitation") + ylab("Golfer Population")
...
splot7<-ggplot(boysVsMornHum,aes(x = x,y = y)) +geom_point(colour='blue') +geom_point(data=girlsVsMornHum,colour='red') + xlab("Morning Humidity") + ylab("Golfer Population")
...
splot8<-ggplot(boysVsAfterHum,aes(x = x,y = y)) +geom_point(colour='blue') +geom_point(data=girlsVsAfterHum,colour='red') + xlab("Afternoon Humidity") + ylab("Golfer Population")
...
## Evaluation of Boys, Girls, and Total Player Population Vs Golf Courses Per State


### Map of Golf Courses in Each State

```{r stateCourse, echo=FALSE}
print(p9)
print(p10)
```

```{r stateCourse1, echo=FALSE}
print(p11)
print(p12)
```


#### While it's hard to tell anything concrete from just observing maps, from observations we can hypothesize that there could be a connection between golf courses and golf popularity. It is interesting to see that there is continued focus of popularity within California, Texas, and Florida.
...
```{r lm5, echo=FALSE}
plot(y ~ x, data = boysVsCourses, ylab = "Golfer Population", xlab = "Golf Courses Per State")
  abline(boysVsCoursesLm)
```
...
```{r lm6, echo=FALSE}
plot(y ~ x, data = girlsVsCourses, ylab = "Golfer Population", xlab = "Golf Courses Per State")
  abline(girlsVsCoursesLm)
```

#### Analyzing Residuals and Testing Regression:

```{r lm6_1, echo=FALSE}
print(summary(girlsVsCoursesLm))
plot(girlsVsCoursesLm)
```
...
#### The amount of golf courses available in each state should be a significant factor when you think about it logically. So to no surprise, the incredibly low p value for both girls and boys data allows us to reject the null hypothesis and claim that errors are correlated. 

#### Based on how far we analyzed the reisuals and regression line, we can say that the number of golf courses open to play in each state is a significant factor that affects golf popularity within that area. The linear regression model seems to be a working fit, but looking at the scatterplot also gives an impression that an exponential regression model might be able to fit the data better. Ultimately, we are still able to conclude that as the number of golf courses increase within an area, so will the number of players. 
