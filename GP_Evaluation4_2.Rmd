...
splot6<-ggplot(boysVsPrecip,aes(x = x,y = y)) +geom_point(colour='blue') +geom_point(data=girlsVsPrecip,colour='red') + xlab("Precipitation (mm)") + ylab("Golfer Population") + ggtitle("Junior Population Vs Avg Annual Precipiation (Boys = Blue, Girls = Red)")
...
## Evaluation of Total Player Population Vs Average Annual Precipitation


### Map of Average Precipitation in millimeters

```{r weatherPrecip, echo=FALSE}
print(p15)
print(p18)
```
...
```{r lm9, echo=FALSE}
plot(y ~ x, data = boysVsTemp, ylab = "Golfer Population", xlab = "Precipitation (mm)")
  abline(boysVsTempLm)
```
...
```{r lm10, echo=FALSE}
plot(y ~ x, data = girlsVsPrecip, ylab = "Golfer Population", xlab = "Precipitation (mm)")
  abline(girlsVsPrecipLm)
```

#### Analyzing Residuals and Testing Regression:

```{r lm10_1, echo=FALSE}
print(summary(girlsVsPrecipLm))
plot(girlsVsPrecipLm)
```
...
#### Weather is another factor that logically seems like a big factor to the popularity of golf. Precipitation is an interesting factor, because people would more likely not want to play in constant rain, but at the same allows golf course maintenaince to cost less. Visually observing the scatterplots with regression lines makes you think that it doesn't fit, and it doesn't, as it fails to reject the null hypothesis of errors correlating, thus meaning that changes in precipitation does not cause changes in golf popularity. However, this is a linear model, and these types of models don't tend to fit weather factors, which would have more of a normal distribution (bell curve) - extremes aren't favored.

#### Based on our analysis of the residuals, we were unable to fit a linear regression relationship onto precipitation and golfer population. Therefore, precipitation either is not a significant factor in the change of golf popularity or has another relationship with golf popularity other than a linear one.
